...
export default async function insertDB(funDescription: string) {
    ...
    try {
        const { data, error } = await supabase.from(DB_TABLE).insert({ created_at: new Date(), description: funDescription, type })
    } catch (error) {
        alert(error)
    }
}